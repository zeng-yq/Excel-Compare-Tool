// Multi-language two column compare page
import ExcelUploadAreaTwoColumn from '@/components/excel-compare/ExcelUploadAreaTwoColumn'
import FAQTwoColumnsI18n from '@/components/FAQTwoColumnsI18n'
import { HowToCompareTwoColumnsI18n } from '@/components/HowToCompareTwoColumnsI18n'
import { RealWorldScenariosTwoColumnsI18n } from '@/components/RealWorldScenariosTwoColumnsI18n'
...
export async function generateMetadata({ params }: { params: { lang: Locale } }): Promise<Metadata> {
  ...
  const seoContent = {
    ...
    zh: {
      title: 'Excel两列对比在线工具 - 免费即时对比',
      description: '在线即时对比Excel两列数据的差异和匹配。无需公式，100%免费安全（客户端处理）。'
    }
  }

  const content = seoContent[params.lang] || seoContent.en
  ...
  return {
    ...
    alternates: {
      canonical: params.lang === 'en'
        ? 'https://excelcompare.org/compare-two-columns'
        : `https://excelcompare.org/${params.lang}/compare-two-columns`
    },
  }
}

export default async function CompareTwoColumns({ params }: { params: { lang: Locale } }) {
    ...
    <div className="container mx-auto py-12 space-y-16">
      <section className="text-center space-y-4">
        <h1 className="text-2xl font-bold tracking-tighter sm:text-3xl md:text-4xl lg:text-5xl">
          <span style={{ color: '#20A884' }}>
            {dict.compareTwoColumns?.hero?.title?.split(' ')[0] || "Compare"}
          </span>{' '}
          <span className="text-gray-700">
            {dict.compareTwoColumns?.hero?.title?.split(' ').slice(1).join(' ') || "Two Excel Columns"}
          </span>
        </h1>
      ...
      {/* Excel 上传区域 */}
      <ExcelUploadAreaTwoColumn />

      {/* How to Compare Two Columns 教程区域 */}
      <HowToCompareTwoColumnsI18n dictionary={dict} />

      {/* Real-World Scenarios 区域 */}
      <RealWorldScenariosTwoColumnsI18n dictionary={dict} />

      {/* FAQ 区域 */}
      <FAQTwoColumnsI18n dictionary={dict} />
    </div>
  )
}